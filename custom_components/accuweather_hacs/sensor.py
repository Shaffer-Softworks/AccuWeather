"""Sensor platform for AccuWeather (HACS)."""
...
async def async_setup_entry(
    ...
    for day in range(min(MAX_FORECAST_DAYS + 1, len(daily))):
        for description in FORECAST_SENSOR_TYPES:
            if description.key in daily[day]:
                sensors.append(
                    AccuWeatherForecastSensor(coordinator, description, day)
                )
...
class AccuWeatherSensor(
    CoordinatorEntity[AccuWeatherHacsCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: AccuWeatherHacsCoordinator,
        ...
        self._sensor_data = self._get_sensor_data(coordinator.data.current, description.key)
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        return self.entity_description.attr_fn(self.coordinator.data.current)
...
class AccuWeatherForecastSensor(
    ...
    def __init__(
        ...
        self.entity_description = description
        self._sensor_data = self._get_sensor_data(
            coordinator.data.daily, description.key, forecast_day
        )
